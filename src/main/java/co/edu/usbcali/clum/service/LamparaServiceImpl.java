...
@Scope("singleton")
@Service
public class LamparaServiceImpl implements LamparaService {
    private static final Logger log = LoggerFactory.getLogger(LamparaServiceImpl.class);
    ...
    */
    ...
    public void validateLampara(Lampara lampara) throws Exception {
        try {
            Set<ConstraintViolation<Lampara>> constraintViolations = validator.validate(lampara);

            if (constraintViolations.size() > 0) {
                StringBuilder strMessage = new StringBuilder();

                for (ConstraintViolation<Lampara> constraintViolation : constraintViolations) {
    ...
    public List<Lampara> getLampara() throws Exception {
        log.debug("finding all Lampara instances");

        List<Lampara> list = new ArrayList<Lampara>();

        try {
            list = lamparaRepository.findAll();
        } catch (Exception e) {
            log.error("finding all Lampara failed", e);
    ...
    public void saveLampara(Lampara entity) throws Exception {
        log.debug("saving Lampara instance");

        try {
            if (entity == null) {
                throw new ZMessManager().new NullEntityExcepcion("Lampara");
            }

            validateLampara(entity);

            if (getLampara(entity.getLamparaId()) != null) {
                throw new ZMessManager(ZMessManager.ENTITY_WITHSAMEKEY);
            }

            lamparaRepository.save(entity);
            log.debug("save Lampara successful");
        } catch (Exception e) {
            log.error("save Lampara failed", e);
            throw e;
        } finally {
        }
    }
    ...
    public void deleteLampara(Lampara entity) throws Exception {
        log.debug("deleting Lampara instance");

        if (entity == null) {
            throw new ZMessManager().new NullEntityExcepcion("Lampara");
        }

        if (entity.getLamparaId() == null) {
            throw new ZMessManager().new EmptyFieldException("lamparaId");
        }
        ...
        try {
            canalizacions = canalizacionRepository.findByProperty("lampara.lamparaId",
                    entity.getLamparaId());
            ...
            lamparaRegistradas = lamparaRegistradaRepository.findByProperty("lampara.lamparaId",
                    entity.getLamparaId());
            ...
            lamparaRepository.deleteById(entity.getLamparaId());
            log.debug("delete Lampara successful");
        } catch (Exception e) {
            log.error("delete Lampara failed", e);
            throw e;
        } finally {
        }
    }
    ...
    public void updateLampara(Lampara entity) throws Exception {
        log.debug("updating Lampara instance");
        ...
        try {
            ...
            lamparaRepository.update(entity);

            log.debug("update Lampara successful");
        } catch (Exception e) {
            log.error("update Lampara failed", e);
            throw e;
        } finally {
        }
    }
    ...
    public List<LamparaDTO> getDataLampara() throws Exception {
        try {
            List<Lampara> lampara = lamparaRepository.findAll();
    ...
    public Lampara getLampara(Integer lamparaId) throws Exception {
        log.debug("getting Lampara instance");

        Lampara entity = null;

        try {
            if (lamparaRepository.findById(lamparaId).isPresent()) {
                entity = lamparaRepository.findById(lamparaId).get();
            }
        } catch (Exception e) {
            log.error("get Lampara failed", e);
            throw new ZMessManager().new FindingException("Lampara");
        } finally {
        }
    ...
    public List<Lampara> findPageLampara(String sortColumnName,
        ...
        throws Exception {
        List<Lampara> entity = null;

        try {
            entity = lamparaRepository.findPage(sortColumnName, sortAscending,
                    startRow, maxResults);
        } catch (Exception e) {
            throw new ZMessManager().new FindingException("Lampara Count");
        } finally {
        }
    ...
    public Long findTotalNumberLampara() throws Exception {
        ...
        try {
            entity = lamparaRepository.count();
        } catch (Exception e) {
            throw new ZMessManager().new FindingException("Lampara Count");
        } finally {
        }
    ...
    * de busqueda voy a hacer.., ejemplo: where nombre=william o where nombre<>william,
    ...
    public List<Lampara> findByCriteria(Object[] variables,
        ...
        try {
            list = lamparaRepository.findByCriteria(where);
        } catch (Exception e) {
